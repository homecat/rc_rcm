 ...
 class Pchart
  {
   ...
   function initalize($XSize,$YSize)
    {
     $this->XSize   = $XSize;
     $this->YSize   = $YSize;
   ...
   function drawScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE)
    {
     ...
     for($i=1;$i<=$Divisions+1;$i++)
      {
       ...
       if ($DataDescription["Format"]["Y"] == "number" )
        $Value = number_format($Value,$Decimals).$DataDescription["Unit"]["Y"];
       ...
       if ( $RightScale )
        {
         imagettftext($this->Picture,$this->FontSize,0,$this->GArea_X2+14,$YPos,$C_TextColor,$this->FontName,$Value);
         if ( $XMin < $this->GArea_X2+15+$TextWidth || $XMin == NULL ) { $XMin = $this->GArea_X2+15+$TextWidth; }
        }
       else
        {
         imagettftext($this->Picture,$this->FontSize,0,$this->GArea_X1-14-$TextWidth,$YPos,$C_TextColor,$this->FontName,$Value);
     ...
     foreach ( $Data as $Key => $Values )
      {    
       if ( ( $this->SkipLabels > 0 && ($ID - 1) % $this->SkipLabels == 0 && $ID < $ColCount-$this->SkipLabels/2 )|| $ID == $ColCount)
        {
         ...
         if ( $Angle == 0 )
          {
           $YPos = $this->GArea_Y2+12+$this->FontSize;
   ...
   function drawGrid($LineWidth,$Mosaic=TRUE,$R=220,$G=220,$B=220,$Alpha=100)
    {
     ...
     for($i=1;$i<=$ColCount;$i++)
      {
       if ( $XPos > $this->GArea_X1 && $XPos < $this->GArea_X2 && ( $this->SkipLabels > 0 && ($i-1) % $this->SkipLabels ==0 ) && $i < $ColCount-$this->SkipLabels/2)
        $this->drawDottedLine(floor($XPos),$this->GArea_Y1,floor($XPos),$this->GArea_Y2,$LineWidth,$R,$G,$B);
       $XPos = $XPos + $this->DivisionWidth;
      }
      $this->drawDottedLine($this->GArea_X1,$this->GArea_Y1,$this->GArea_X2,$this->GArea_Y1,$LineWidth,$R,$G,$B);
      $this->drawDottedLine($this->GArea_X2,$this->GArea_Y1,$this->GArea_X2,$this->GArea_Y2,$LineWidth,$R,$G,$B);
    }
